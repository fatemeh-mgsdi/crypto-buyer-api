...
def check_pending_transactions(sender, instance, created, **kwargs):
    total_amounts = PendingTransaction.objects.select_related('crypto').values('crypto__name').annotate(total=Sum('amount'))

    for total_amount in total_amounts:
        if total_amount['total'] >= 10:
            buy_from_exchange(total_amount['crypto__name'], total_amount['total'])
            PendingTransaction.objects.filter(crypto__name=total_amount['crypto__name']).delete()